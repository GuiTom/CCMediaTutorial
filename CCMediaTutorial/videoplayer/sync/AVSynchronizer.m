...
- (void) decodeFramesWithDuration:(float) duration;
{
    BOOL good = YES;
    while (good) {
        good = NO;
...
- (OpenState) openFile: (NSString *) path usingHWCodec: (BOOL) usingHWCodec parameters:(NSDictionary*) parameters error: (NSError **) perror;
{
    ...
    if (ABS(_minBufferedDuration - 0.f) < 0) {
        if(isNetwork){
    ...
    if ((ABS(_maxBufferedDuration - 0.f) < 0)) {
        if(isNetwork){
...
- (VideoFrame*) getCorrectVideoFrame;
{
    ...
    @synchronized(_videoFrames) {
        while (_videoFrames.count > 0) {
            ...
            const float delta = _audioPosition - frame.position;
...
- (BOOL) addFrames: (NSArray *)frames duration:(float) duration
{
